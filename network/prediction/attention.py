...
class Attention(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, num_classes, device=torch.device('cuda')):
    ...
    def forward(self, batch_H, text, is_train=True, batch_max_length=25, **kwargs):
        """
        input:
            batch_H : contextual_feature H = hidden state of encoder. [batch_size x num_steps x contextual_feature_channels]
            text : the text-index of each image. [batch_size x (max_length+1)]. +1 for [GO] token. text[:, 0] = [GO].
            batch_max_length (int): max text length of a prediction
        output: probability distribution at each step [batch_size x num_steps x num_classes]
        """
        batch_H = batch_H.contiguous()
...
def attn(opt):
    return Attention(opt.n_features, opt.hidden_size, opt.num_classes, opt.device)
